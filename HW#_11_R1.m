...
WF_l = EA_l + BG_l/2 + kb*T/q *log(Na_l/ni_l);
WF_r = EA_r + BG_r/2 - kb*T/q *log(Nd_r/ni_r);

Vbi = WF_l - WF_r - Va;

% Part on P-side, xp

xp = sqrt((2*perm_a*perm_r_l*Vbi/q)*(Nd_r/(Na_l*(Nd_r+Na_l)))); % p-type depletion layer width

% Part on N-side, xn

xn = sqrt((2*perm_a*perm_r_r*Vbi/q)*(Na_l/(Nd_r*(Na_l+Nd_r)))); % n-type depletion layer width
...
V_xn_CB = (q*Nd_r/(perm_a*perm_r_r)*(xn*x-x.^2/2)+q*Na_r/(2*perm_r_r*perm_a)*xp.^2).*(x >= 0 & x < xn)...
   - (q*Na_r/(2*perm_r_r*perm_a)*xp.^2).*Delta_EA.*(x==0) + (((q*Nd_r/(perm_a*perm_r_r)*(xn*xn-xn.^2/2)) +(q*Na_r/(2*perm_r_r*perm_a)*xp.^2))).*(x>xn);

% Calculating V_xp_VB where -xp ≤ x ≤ 0

V_xp_VB = 0.*(x<xp) + (q*Na_l/(2*perm_r_r*perm_a)*xp.^2).*(q*Na_l/(2*perm_r_l*perm_a)*(x+xp).^2).*(x > -xp & x < 0);

% Calculating V_xn_VB where 0 ≤ x ≤ xn

V_xn_VB =  Delta_EAnEG.*(q*Nd_r/(perm_a*perm_r_r)*(xn*x-x.^2/2)+q*Na_r/(2*perm_r_r*perm_a)*xp.^2).*(x >= 0 & x < xn)...
    + Delta_EAnEG.*(((q*Nd_r/(perm_a*perm_r_r)*(xn*xn-xn.^2/2)) +(q*Na_r/(2*perm_r_r*perm_a)*xp.^2))).*(x>xn);
